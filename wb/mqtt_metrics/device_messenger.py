...
class MqttMessenger:
    ...
    def _track(self, topic):
        ...
        # is registered again and again. Publishing a retained topic twice is idempotent,
        # appending it twice is not: without this guard the list grows without bound and
        # remove_device() clears every topic once per connect.
        if topic not in self.cleanup_topics:
    ...
    def create_control(self, metric_name, ctrl_meta):
        ...
        self._track(f"/devices/{self.device_name}/controls/{metric_name}")
